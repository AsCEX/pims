(function($)
{
	jQuery.fn.grid = function(options, callback)
	{
		return this.each(function()
		{
		    ...
		    initGrid = function()
		    {
				...
				columnCount = $(headerGrid).children().length;  													// get column count of header
				...
				gridLoading = '<tr><td colspan="'+columnCount+'" class="grid-loading text-center"><small class="loader"><img src="' + site_url + 'assets/dist/img/ajax-loader.gif" /></small></td></tr>';	// create loading grid row
			...
			layoutGrid = function()
			{
				...
				var containerGrid = '<tbody class="inner-grid"></tbody>';
				...
				$(gridEmpty).appendTo($(obj).find('tbody.inner-grid'));										// append empty message grid to inner grid
				$(gridLoading).appendTo($(obj).find('tbody.inner-grid'));										// append loading animation to inner grid
						...
						success: function(data) {
							...
							if (data['status']=='error') {
								...
								if (currentPage>=info.totalPages){
									...
									$(obj).find('.grid-controls .paging').show();								// hide paging links
									$(obj).find('.grid-controls .page-nav').hide();							// hide paging links
								}else{
									obj.find('.grid-controls .page-next').show();    							// hide next page link
											...
											$.each ($(newrow).find('.'+key), function(e){
												if($(this).is('input')){
													if($(this).attr('type')=='checkbox'){						// if element is checkbox
														...
														}		// if element is other input element
													}else{
							...
							if(typeof callback != "undefined")
								callback(grid_events, data);


							$(obj).find('.delete-row').on('click',function(){
								...
								$.ajax({
									...
									success: function(data){
										if(data.status == 'success'){
											...
											$(".alert").slideDown().delay(5000).fadeOut();
							...
							$(obj).find('.restore-row').on('click',function(){
								$(this).css('opacity', '0.5');
								console.log("remove");
								that = this;
